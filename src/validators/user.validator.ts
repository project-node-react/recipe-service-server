...
export const FollowersResponseSchema = registry.register(
  ...
  z.object({
    ...
    totalPages: z.number().int().min(1).openapi({ example: 5 }), // Додали поле пагінації
    currentPage: z.number().int().min(1).openapi({ example: 1 }), // Додали поле пагінації
  }),
);
...
registry.registerPath({
  ...
  request: {
    params: UserIdParamsSchema,
    query: PaginationQuerySchema, // Додали валідацію ?page=1&limit=9
  },
  responses: {
    200: {
...
export const FollowingResponseSchema = registry.register(
  ...
  z.object({
    following: z.array(
      z.object({
        id: z.string().openapi({ example: "64c8d958249fae54bae90bb9" }),
        name: z.string().openapi({ example: "petro_vasylenko" }),
        avatar: z.string().nullable().openapi({
          example: "https://cloudinary.com",
        }),
      }),
    ),
    totalPages: z.number().int().min(1).openapi({ example: 3 }), // Додали поле пагінації
    currentPage: z.number().int().min(1).openapi({ example: 1 }), // Додали поле пагінації
  }),
);
