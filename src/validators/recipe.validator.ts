...
export const CreateRecipeSchema = registry.register(
  ...
  z.object({
    ...
    time: z.coerce.number().int().min(1, 'cooking time must be at least 1 minute'),
...
export const RecipeCardSchema = registry.register(
  ...
  z.object({
    ...
    description: z.string().nullable().openapi({ example: 'Traditional Ukrainian beet soup' }),
    thumb: z.string().nullable().openapi({
      example: 'https://res.cloudinary.com/dzxdn99qc/image/upload/v1/recipes/borscht.jpg',
    }),
...
export const RecipeIngredientSchema = registry.register(
  ...
  z.object({
    ...
    img: z.string().nullable().openapi({
      example: 'https://res.cloudinary.com/dzxdn99qc/image/upload/v1/ingredients/beetroot.jpg',
    }),
...
export const RecipeDetailSchema = registry.register(
  ...
  z.object({
    ...
    instructions: z.string().openapi({ example: 'Boil the beets, chop the vegetables...' }),
...
registry.registerPath({
  ...
  responses: {
    200: {
      ...
      content: { 'application/json': { schema: paginatedResponse(RecipeCardSchema) } },
    },
...
registry.registerPath({
  ...
  request: { params: z.object({ id: z.string().openapi({ example: '64c8d958249fae54bae90bb9' }) }) },
  responses: {
    200: {
...
registry.registerPath({
  ...
  description:
    'Multipart form data. `ingredients` must be sent as a JSON-encoded string, ' +
    'e.g. \'[{"id":"ing-beetroot","measure":"2 pcs"}]\'. `thumb` is an optional image file.',
  security: [{ bearerAuth: [] }],
  request: {
    body: {
  ...
  responses: {
    ...
    400: { description: 'Unknown category/area/ingredient id', content: { 'application/json': { schema: ErrorSchema } } },
    401: { description: 'Authentication required', content: { 'application/json': { schema: ErrorSchema } } },
    422: { description: 'Validation error' },
  },
});
